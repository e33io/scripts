...
echo "#########################################################"
echo "Update deb-post-install-jwm.sh script"
echo "#########################################################"

sed -i 's/sudo rm -R \$HOME\/scripts/#/' $HOME/scripts/deb-post-install-jwm.sh

echo "#########################################################"
echo "Run deb-post-install-jwm.sh script to"
echo "install jwm and other applications"
echo "#########################################################"
...
sed -i 's/background\.png/background-1\.jpg/' $HOME/.config/nitrogen/bg-saved.cfg
sed -i 's/yoffset: -275;/\/\* yoffset: -275; \*\//' $HOME/.config/rofi/config.rasi
sed -i 's/yoffset: -180;/\/\* yoffset: -180; \*\//' $HOME/.config/rofi/config.rasi
...
echo "#########################################################"
echo "Add required lightdm-xsession.desktop file"
echo "#########################################################"
...
Exec=default
Icon=
Type=Application" | sudo tee /usr/share/xsessions/lightdm-xsession.desktop > /dev/null
