...
cp -R ~/core/home/.??* ~/
cp -R ~/core/arch/home/.??* ~/
cp -R ~/scripts/set-theming-dk.sh ~/.local/bin/set-theming-dk
sudo cp -R ~/core/root/* /
...
sed -i -e '/libexec/d' -e 's/#\/usr\/lib/\/usr\/lib/' -e 's/#xbindkeys/xbindkeys/' ~/.config/dk/dkrc
sed -i 's/brave-browser/brave/' ~/.config/dk/sxhkdrc
...
if ! grep -q 'XDG_CURRENT_DESKTOP=dk' ~/.profile; then
