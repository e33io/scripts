...
sudo pacman -S --noconfirm --needed xorg-server xorg-apps sxhkd polybar i3lock xss-lock \
dmenu rofi dunst base-devel upower gvfs nfs-utils cifs-utils libpulse libmad rsync cronie \
git curl wget tar less 7zip xsel xclip xbindkeys xdotool playerctl dex xdg-desktop-portal-gtk \
xdg-user-dirs mate-polkit lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings feh \
lxappearance gnome-themes-extra qt5ct qt6ct ttf-dejavu noto-fonts-emoji papirus-icon-theme \
breeze-icons plymouth pavucontrol-qt kitty python-pillowfight xterm bash-completion vim nano \
...
engrampa scrot atril imv mpv parole mousepad galculator timeshift signal-desktop gpick \
...
cp -R $HOME/extra/dk/.config $HOME
