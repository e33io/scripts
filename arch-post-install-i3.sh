...
sudo pacman -S --noconfirm --needed xorg-server xorg-apps i3-wm i3status i3lock python-i3ipc py3status xss-lock dmenu \
rofi dunst gvfs nfs-utils cifs-utils fuse rsync cronie git curl wget tar less 7zip base-devel xsel xclip playerctl \
...
noto-fonts-emoji papirus-icon-theme breeze-icons plymouth pavucontrol-qt nitrogen kitty python-pillowfight xterm \
thunar thunar-archive-plugin thunar-volman tumbler ffmpegthumbnailer engrampa scrot atril imv mpv parole mousepad \
galculator dconf-editor gnome-disk-utility timeshift bash-completion vim nano micro fzf lazygit htop fastfetch cmus \
cava ranger ueberzug highlight atool w3m mediainfo perl-image-exiftool signal-desktop gpick darktable gimp inkscape \
...
sudo mv /usr/share/wayland-sessions/weston.desktop \
/usr/share/wayland-sessions/weston.desktop.bak
