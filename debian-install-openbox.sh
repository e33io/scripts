...
sudo apt -y install openbox polybar network-manager i3lock rofi dunst playerctl xssproxy xsel xclip xinput \
...
heif-gdk-pixbuf gvfs-fuse gvfs-backends nfs-common cifs-utils engrampa pipewire-audio pulseaudio-utils pavucontrol-qt \
synaptic dconf-cli mintstick scrot atril imv mpv parole mousepad galculator gpick darktable gimp inkscape filezilla \
libreoffice-calc libreoffice-draw libreoffice-impress libreoffice-writer libreoffice-gtk3 timeshift xterm lazygit \
fastfetch htop cmus cava cmatrix ncal micro ranger ueberzug caca-utils highlight atool w3m poppler-utils mediainfo \
fzf libimage-exiftool-perl apt-transport-https curl rsync wmctrl xdotool xbindkeys
...
sed -i '/at-spi/d' $HOME/.config/openbox/autostart # && sed -i '31d' $HOME/.config/openbox/autostart
...
echo "Openbox was installed via e33io script: $(date '+%B %d, %Y, %H:%M')" \
