...
# URL: https://github.com/e33io/scripts/blob/main/set-theming-dk.sh
# -----------------------------------------------------------------------------
# Use this script at your own risk, it will overwrite existing files!
# NOTE: Only use with dk installed via e33io script!
# =============================================================================
...
theming_files () {
    # dk window colors
    sed -i -e "s/^    focus='.*'/    focus='$dk_focus'/" \
    -e "s/^    unfocus='.*'/    unfocus='$dk_unfocus'/" \
    -e "s/^    urgent='.*'/    urgent='$dk_urgent'/" ~/.config/dk/dkrc
    # Polybar colors
    sed -i -e "s/bg = .*/bg = $bar_bg/" \
    -e "s/bg-act = .*/bg-act = $accent_color/" \
    -e "s/bg-occ = .*/bg-occ = $bar_bg/" \
    -e "s/fg = .*/fg = $bar_fg/" \
    -e "s/\%{F.*}\|\%{F-}/\%{F$accent_color}\|\%{F-}/" ~/.config/dk/polybar/config.ini
    ...
    sed -i "s/-n -c .* \&/-n -c $lock_bg_color \&/" ~/.config/dk/dkrc
